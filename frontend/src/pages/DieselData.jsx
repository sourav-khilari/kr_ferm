...
const emptyForm = {
  date: '', slNo: '', vehicleNo: '', product: '', qty: '', rate: '', amount: ''
};
...
export default function DieselData() {
  ...
  const [filters, setFilters] = useState({
    vehicleNo: '', validationStatus: '', dateFrom: '', dateTo: '',
    uploadRun: runId || ''
  });
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get('/diesel/records', { params });
      if (res.data.success) {
  ...
  const handleEditFieldChange = (field, value) => {
    ...
    if (!isNaN(qty) && !isNaN(rate) && !isNaN(amount)) {
      ...
      if (Math.abs(amount - calcAmount) > 0.05) errs.push(`Amount mismatch: expected ${calcAmount.toFixed(2)}`);
    }

    if (!updated.vehicleNo) errs.push('Missing Vehicle No.');
    setEditErrors(errs);
  ...
  const handleSaveEdit = async () => {
    setSaving(true);
    try {
      const res = await api.put(`/diesel/records/${editRow._id}`, editForm);
      if (res.data.success) {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      const res = await api.delete(`/diesel/records/${deleteTarget._id}`);
      if (res.data.success) {
  ...
  const columns = [
    ...
    { field: 'pump', headerName: 'Pump', width: 130, valueGetter: (value) => value?.name || '—' },
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
        <Box>
          <Typography variant="h5" sx={{ fontWeight: 700 }}>Diesel Logs Directory</Typography>
          <Typography variant="body2" color="text.secondary">
            {runName ? `Showing records for: ${runName}` : 'View, edit, or delete uploaded diesel invoice rows.'}
          </Typography>
        </Box>
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        ...
        <Box sx={{ mt: 1.5, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
          ...
          {(filters.vehicleNo || filters.validationStatus || filters.dateFrom || filters.dateTo) && (
            <Chip
              label="Clear filters"
              size="small"
              variant="outlined"
              onDelete={() => setFilters({ vehicleNo: '', validationStatus: '', dateFrom: '', dateTo: '', uploadRun: runId || '' })}
            />
          )}
        </Box>
      </Paper>
      ...
      <Dialog open={Boolean(deleteTarget)} onClose={() => setDeleteTarget(null)}>
        ...
        <DialogContent>
          <Typography>
            ...
            {deleteTarget && <> Sl No: <strong>{deleteTarget.slNo || '—'}</strong>, Vehicle: <strong>{deleteTarget.vehicleNo}</strong></>}
          </Typography>
        </DialogContent>
        <DialogActions>
